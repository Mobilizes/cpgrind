// Problem: Chessboard and Queens
// Contest: CSES Problem Set
// Judge: CSES
// URL: https://cses.fi/problemset/task/1624
// Memory Limit: 512
// Time Limit: 1000
// Start: 03-12-2025 16:51:25 WIB

#include <bits/stdc++.h>

typedef long long ll;

using namespace std;

#define Mob                         \
  ios_base::sync_with_stdio(false); \
  cin.tie(NULL);
#define el "\n"
#define pii pair<int, int>
#define pll pair<ll, ll>
#define all(a) begin(a), end(a)

const bool is_testcases = false;
const bool desync_stdio = false;
const int floating_precision = 8;

int diri[] = {1, -1, 0, 0, 1, 1, -1, -1};
int dirj[] = {0, 0, 1, -1, 1, -1, 1, -1};

const int sz = 8;

void move(int i, int j, int val, int diridx, vector<vector<int>> & pos) {
  if (i < 0 || i >= sz || j < 0 || j >= sz) return;
  pos[i][j] += val;
  move(i + diri[diridx], j + dirj[diridx], val, diridx, pos);
}

int rec(int a, int b, int pc, vector<vector<int>> & pos) {
  if (pc == sz) {
    return 1;
  }

  int res = 0;

  for (int i=0; i<sz; i++) {
    for (int j=0; j<sz; j++) {
      if (pos[i][j]) continue;
      if (i < a || j < b) continue;
      for (int k=0; k<8; k++) {
        move(i+diri[k], j+dirj[k], 1, k, pos);
      }

      pos[i][j]++;
      res += rec(i, j, pc+1, pos);
      pos[i][j]--;

      for (int k=0; k<8; k++) {
        move(i+diri[k], j+dirj[k], -1, k, pos);
      }
    }
  }

  return res;
}

auto solve(int tecs = -1)
{
  vector<string> arr(8);
  for (int i=0; i<8; i++) cin >> arr[i];

  vector<vector<int>> pos(8, vector<int>(8, 0));
  for (int i=0; i<8; i++) {
    for (int j=0; j<8; j++) {
      if (arr[i][j] == '.') continue;
      pos[i][j]++;
    }
  }

  return rec(0, 0, 0, pos);
}

void initialize()
{
}

template <typename T>
void call_solve(T f, int tecs = -1)
{
  using ReturnValue = invoke_result_t<decltype(f), int>;
  if constexpr (is_same_v<ReturnValue, bool>) {
    cout << (f(tecs) ? "YES" : "NO") << el;
  } else if constexpr (is_integral_v<ReturnValue> || is_same_v<ReturnValue, string> ||
                       is_same_v<ReturnValue, const char *>) {
    cout << f(tecs) << el;
  } else if constexpr (is_floating_point_v<ReturnValue>) {
    cout << fixed << setprecision(floating_precision) << f(tecs) << el;
  } else if constexpr (ranges::range<ReturnValue>) {
    for (auto i : f(tecs)) cout << i << ' ';
    cout << el;
  } else {
    f(tecs);
  }
}

int main()
{
  if constexpr (desync_stdio) {
    Mob;
  }

  initialize();
  if constexpr (is_testcases) {
    int testcase;
    cin >> testcase;
    for (int tecs = 1; tecs <= testcase; tecs++) {
      call_solve(solve, tecs);
    }
  } else {
    call_solve(solve);
  }
}
// if you see good syntax format, i used clang-format
